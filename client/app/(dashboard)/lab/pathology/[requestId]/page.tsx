...
import { ArrowLeft, User, FlaskConical, FileText, Calendar, Activity, Download } from 'lucide-react';
import { useGetPathologyRequestQuery } from '@/store/api/lab.api';
...
export default function PathologyDetailPage({ params }: { params: { requestId: string } }) {
  const { requestId } = params;
  const { data: request, isLoading, isError } = useGetPathologyRequestQuery(requestId);
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">Pathology Request</h1>
      ...
      <div className="rounded-xl border bg-card divide-y">
        ...
        <DetailRow icon={<FlaskConical className="h-4 w-4" />} label="Test Type"    value={request.testType} />
