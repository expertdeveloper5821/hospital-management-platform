...
- IPD 404 bug fixed — `dischargePatient` empty `admissionId` produced double-slash URL `/api/ipd/admissions//discharge` hitting the 404 catch-all; `handleSave` empty `bedId` bug fixed; defensive guards added to all IPD mutation query builders and `listBeds` query (2026-06-12)
- IPD `updateAdmission` dist rebuild — `PATCH /api/ipd/admissions/:admissionId` route and department-scoped `listAdmissions` were present in TypeScript source but absent from compiled `dist/`; ran `npm run build` to regenerate; server restart required (2026-06-12)
- Nurse assignment to wards — HOSPITAL_ADMIN and DOCTOR can assign/remove nurses per ward via chip add/remove UI in Wards page; `IWard.assignedNurseIds: string[]` added; `PATCH /api/ipd/wards/:wardId/nurses` endpoint; each nurse validated as NURSE role; nurse count badge shown on collapsed ward row (2026-06-19)
- Wards module added to sidebar — new `/wards` page with Wards & Beds and Occupancy Summary tabs; WardsTab/OccupancyTab/WardRow/CreateWardModal/AddBedsModal components moved from IPD page; IPD page simplified to Admissions only; sidebar nav updated for HOSPITAL_ADMIN, ADMIN, MANAGER, DOCTOR, NURSE, RECEPTIONIST (2026-06-19)
- Mandatory payment on OPD visit creation — Amount (₹) + Payment Mode (Cash/UPI/Card) required; auto-creates `COMPLETED` manual payment (description: "OPD Consultation – Visit #N"); VisitPanel view mode shows payment amount and mode (2026-06-18)
- Mandatory payment on IPD admission creation — Amount (₹) + Payment Mode (Cash/UPI/Card) required; auto-creates `COMPLETED` manual payment (description: "IPD Admission"); AdmissionPanel view mode shows payment amount and mode (2026-06-18)
- Optional registration fee on patient creation — Registration Type toggle (Free/Paid); Paid requires Fee (₹) + Payment Mode (Cash/UPI/Card); stores `registrationFee` + `registrationPaymentMethod` on patient; auto-creates `COMPLETED` manual payment (description: "Patient Registration Fee"); Medical Card PDF footer shows fee; patient detail panel shows Fee/Payment Mode (2026-06-18)
- Multi-doctor support on OPD visits and IPD admissions — `IOPDVisit.doctorId: string | null` replaced by `doctorIds: string[]`; `IIPDAdmission.assignedDoctorId: string | null` replaced by `assignedDoctorIds: string[]`; backend schemas, services, repositories, controllers, Zod validators, frontend types, OPD/IPD page UI (multi-doctor chip add/remove pattern), and all affected tests updated (2026-06-18)
- Doctor-scoped OPD + IPD filtering — DOCTOR role now sees only their own visits/admissions (filtered by `doctorIds: { $in: [userId] }` / `assignedDoctorIds: { $in: [userId] }`) instead of all records across their departments (2026-06-12)
