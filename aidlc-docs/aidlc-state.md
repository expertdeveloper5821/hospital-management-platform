...
- Doctor-scoped OPD + IPD filtering — DOCTOR role now sees only their own visits/admissions (filtered by `doctorId`/`assignedDoctorId`) instead of all records across their departments; `userRepository` department lookup removed from both OPD and IPD controllers; dist rebuilt (2026-06-12)
