...
const createPatientSchema = z.object({
  ...
  address:                   z.string().min(10, 'Address must be at least 10 characters.').max(300, 'Address cannot exceed 300 characters.').trim(),
...
const updatePatientSchema = z.object({
  ...
  address:                z.string().min(10, 'Address must be at least 10 characters.').max(300, 'Address cannot exceed 300 characters.').trim().optional(),
  aadhaarNumber:          z.string().length(12).regex(/^[0-9]+$/).optional(),
  emergencyContactName:   z.string().min(1).max(200).optional(),
  emergencyContactMobile: z.string().min(7).max(15).regex(/^\+?[0-9]+$/).optional(),
  bloodGroup:             z.enum(BLOOD_GROUP_VALUES).optional(),
