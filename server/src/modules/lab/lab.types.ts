...
export const CreatePathologyRequestSchema = z.object({
  ...
  notes:     z.string().max(12000, 'Notes content is too large.').trim().refine(notesMaxCheck, NOTES_TOO_LONG).optional(),
});
...
export const EditPathologyRequestSchema = z.object({
  ...
  notes:    z.string().max(12000, 'Notes content is too large.').trim().refine(notesMaxCheck, NOTES_TOO_LONG).nullable().optional(),
