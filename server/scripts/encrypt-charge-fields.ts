...
// One-off migration: encrypts the Charge `description` free-text field written
...
//
// Usage:
//   npm run migrate:encrypt-charge-fields            # apply
//   npm run migrate:encrypt-charge-fields -- --dry-run
...
export async function migrateChargeFields(
  ...
  console.log(`Found ${total} charge(s) with an unencrypted description field.`);
  if (opts.dryRun) {
  ...
  console.log(`Encrypted description on ${updated} charge(s).`);
  return updated;
}
...
async function main(): Promise<void> {
  ...
  try {
    await migrateChargeFields(ChargeModel.collection, { dryRun: process.argv.includes('--dry-run') });
  } finally {
...
if (require.main === module) {
  ...
  main().catch((err) => {
    console.error('encrypt-charge-fields failed:', err instanceof Error ? err.message : 'unknown error');
    process.exit(1);
  });
}
