...
// One-off migration: encrypts Payment free-text / reference fields written
// before field-level encryption covered them — `description` and `transactionId`.
//
...
//
// Usage:
//   npm run migrate:encrypt-payment-fields            # apply
//   npm run migrate:encrypt-payment-fields -- --dry-run

const FIELDS = ['description', 'transactionId'] as const;
const BATCH_SIZE = 200;
const KEY = EncryptionKeyPurpose.PAYMENT;
...
export async function migratePaymentFields(
  ...
  console.log(`Found ${total} payment(s) with at least one unencrypted field.`);
  if (opts.dryRun) {
  ...
  console.log(`Encrypted fields on ${updated} payment(s).`);
  return updated;
}
...
async function main(): Promise<void> {
  ...
  try {
    await migratePaymentFields(PaymentModel.collection, { dryRun: process.argv.includes('--dry-run') });
  } finally {
...
if (require.main === module) {
  ...
  main().catch((err) => {
    console.error('encrypt-payment-fields failed:', err instanceof Error ? err.message : 'unknown error');
    process.exit(1);
  });
}
