...
// One-off migration: encrypts every OPDVisit `vitals.*` value written before
// field-level encryption covered it (weight/height/sugar/bodyTemperature —
// numeric — and bloodPressure — string).
//
...
// encrypted the next time its visit's vitals are written. This converts them
// at rest instead of waiting.
//
...
// re-asserts the visit's exact prior vitals object as a concurrency guard, so
// a concurrent vitals edit from the running app is never overwritten.
//
// Usage:
//   npm run migrate:encrypt-opd-vitals            # apply
//   npm run migrate:encrypt-opd-vitals -- --dry-run

const BATCH_SIZE = 200;
const KEY = EncryptionKeyPurpose.MEDICAL;
const NUMBER_FIELDS = ['weight', 'height', 'sugar', 'bodyTemperature'] as const;

// Matches a visit holding at least one legacy-plaintext vitals value: a real
// BSON number in one of the numeric fields, or a non-empty string not
...
export async function migrateOpdVitals(
  ...
  console.log(`Encrypted vitals on ${updated} OPD visit(s).`);
  return updated;
}
...
async function main(): Promise<void> {
  ...
  try {
    await migrateOpdVitals(OPDVisitModel.collection, { dryRun });
  } finally {
...
if (require.main === module) {
  ...
  main().catch((err) => {
    console.error('encrypt-opd-vitals failed:', err instanceof Error ? err.message : 'unknown error');
    process.exit(1);
  });
}
