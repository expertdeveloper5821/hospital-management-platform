...
describe('PatientService — example-based', () => {
  ...
  describe('searchPatients', () => {
    test('delegates to repository with correct tenantId', async () => {
      ...
      expect(mockRepo.search).toHaveBeenCalledWith('t1', 'Ravi', 1, 20, undefined, undefined);
    ...
    test('passes undefined q when no search term', async () => {
      ...
      expect(mockRepo.search).toHaveBeenCalledWith('t1', undefined, 1, 20, undefined, undefined);
    });
  });
